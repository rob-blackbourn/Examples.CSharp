...
namespace JetBlack.Examples.SocketClient1
{
    class Program
    {
        public static void Main(string[] args)
        {
            ...
            {
                client.Dispatch(splitArgs[0], int.Parse(splitArgs[1]), cts.Token);
            }
        }
    }
    ...
    class EchoClient : IDisposable
    {
        ...
        public void Dispatch(string hostname, int port, CancellationToken token)
        {
            ...
            for (var line = Console.ReadLine(); !token.IsCancellationRequested && !string.IsNullOrEmpty(line); line = Console.ReadLine())
            {
                ...
                while (written < writeBuffer.Length)
                    written += _socket.Send(writeBuffer, written, writeBuffer.Length - written, SocketFlags.None);

                var readBuffer = new byte[written];
                var read = 0;
                while (read < written)
                    read += _socket.Receive(readBuffer, read, written - read, SocketFlags.None);
